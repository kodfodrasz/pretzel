...
namespace Pretzel.Logic.Extensibility.Extensions
{
    public class AzureHostSupport : IAdditionalIngredient, IHaveCommandLineArgs
    {
        ...
        public void MixIn(string directory)
        {
            ...
            {
                var trimStart = file.Replace(directory, string.Empty).TrimStart(fileSystem.Path.DirectorySeparatorChar, fileSystem.Path.AltDirectorySeparatorChar);
            ...
            {
                var trimStart = directoryToMove.Replace(directory, string.Empty).TrimStart(fileSystem.Path.DirectorySeparatorChar, fileSystem.Path.AltDirectorySeparatorChar);
